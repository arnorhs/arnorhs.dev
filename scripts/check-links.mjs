...
async function main() {
  ...
  console.log(`Found ${urls.length} unique links (${[...linkOccurrences.values()].flat().length} occurrences)`);
  ...
  const results = await runWithConcurrency(urls, config.concurrency, (url) => checkOneUrl(url, config));
